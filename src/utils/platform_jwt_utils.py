...
oauth2_scheme_platform = OAuth2PasswordBearer(tokenUrl="/api/v1/platform-auth/token")
load_dotenv()

JWT_SECRET_KEY = os.getenv("JWT_SECRET_KEY", "sadfasdfsadfsadfsadf")

def encode_platform_token(payload: dict):
    payload["exp"] = datetime.now(timezone.utc) + timedelta(hours=1)
    token = jwt.encode(payload, JWT_SECRET_KEY, algorithm="HS256")
    return token

def decode_platform_token(token: Annotated[str, Depends(oauth2_scheme_platform)], db: SessionDep):
    try:
